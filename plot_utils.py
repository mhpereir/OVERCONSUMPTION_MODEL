...
class plot_results:
    def __init__(self, plotting_flag, model, z_init, z_final): 
    ...
    def delay_times(self, model, z_init, z_final):
        ...
        fig,ax = plt.subplots()
        contourf_ = ax.tricontourf(xx,yy,zz, np.arange(0,14,0.1), extend='both')
        ...
        ax.set_ylim([8,12])
        #cbar      = fig.colorbar(contourf_, label='Quenching Efficiency')
        ...
        fig,ax = plt.subplots()
        contourf_ = ax.tricontourf(xx_2,yy,zz, np.arange(0,14,0.1), extend='both')
        ...
        temp_m      = np.ma.copy(model.infall_m)
